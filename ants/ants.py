...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place, step = self.place, 0
        while len(place.bees) == 0 or step < self.min_range or step > self.max_range:
            step += 1
            if place.entrance != beehive:
                place = place.entrance
            else:
                break
        if step < self.min_range or step > self.max_range:
            return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        def damage_bees(amout):
            for bee in self.place.bees.copy():
                bee.reduce_armor(amout)
        
        damage_bees(amount)
        if amount >= self.armor:
            damage_bees(self.damage)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.is_impostor = QueenAnt.queen_count != 0
    ...
    def action(self, gamestate):
        ...
        if self.is_impostor:
            ...
            while place != None:
                if place.ant != None:
                    ants_behind.append(place.ant)
                    if hasattr(place.ant, "contained_ant") and place.ant.contained_ant != None:
                        ants_behind.append(place.ant.contained_ant)
                place = place.exit
            
            for ant in ants_behind:
                if not hasattr(ant, "has_been_doubled"):
                    ant.has_been_doubled = True
                    ant.damage *= 2
        # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if hasattr(self, 'scared') and self.scared: destination = self.place.entrance
        # END EC
        if self.blocked():
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    old_action, new_action = bee.action, status(bee.action, bee)
    def action(gamestatus):
        nonlocal length
        if length > 0:
            new_action(gamestatus)
            length -= 1
        else:
            old_action(gamestatus)
    bee.action = action
    # END Problem EC
