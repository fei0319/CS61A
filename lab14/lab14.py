...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        match, cost = '+[{}]'.format(code[code_idx]) + match, cost + 1
        possibilities.append((match, cost))
        # Delete
        match, cost = helper_align(skeleton_idx + 1, code_idx)
        match, cost = '-[{}]'.format(skeleton[skeleton_idx]) + match, cost + 1
        possibilities.append((match, cost))
...
def num_splits(s, d):
    ...
    for i in s:
        nxt = dict()
        for dif in dp:
            if dif + i not in nxt: nxt[dif + i] = 0
            if dif - i not in nxt: nxt[dif - i] = 0
            nxt[dif + i] += dp[dif]
            nxt[dif - i] += dp[dif]
        dp = nxt
...
def insert(link, value, index):
    ...
    IndexError
    """
    if link is Link.empty or (index == 1 and link.rest is Link.empty):
        raise IndexError
    if index == 1:
        link.rest = Link(value, link.rest)
    elif index == 0:
        def copy(link):
            if link is Link.empty: return Link.empty
            return Link(link.first, copy(link.rest))
        link.first, link.rest = value, copy(link)
    else:
