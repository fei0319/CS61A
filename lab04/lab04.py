...
def reverse_recursive(lst):
    ...
    last_element = lst[-1]
    lst.pop()
    return [last_element] + reverse_recursive(lst)
...
def distance(city_a, city_b):
    ...
    def eculid_d(x, y):
        return sqrt(x * x + y * y)
    return eculid_d(get_lat(city_a) - get_lat(city_b), get_lon(city_a) - get_lon(city_b))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    distance_a, distance_b = distance(city_a, make_city('', lat, lon)), distance(city_b, make_city('', lat, lon))
