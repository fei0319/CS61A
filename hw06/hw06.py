...
class VendingMachine:
    ...
    def vend(self):
        ...
        self.funds = 0
        self.stock -= 1
        return 'Here is your {}'.format(self.name) + (' and ${} change.'.format(change) if change != 0 else '.')
...
def is_bst(t):
    ...
    "*** YOUR CODE HERE ***"
    def bst(t, f):
        return f([bst(b, f) for b in t.branches] + [t.label])
    
    if len(t.branches) == 0:
        return True
    elif len(t.branches) == 1:
        return (bst(t.branches[0], min) > t.label or bst(t.branches[0], max) <= t.label) and is_bst(t.branches[0])
    elif len(t.branches) == 2:
        return bst(t.branches[0], max) <= t.label and bst(t.branches[1], min) > t.label and all([is_bst(b) for b in t.branches])
    else:
        return False
