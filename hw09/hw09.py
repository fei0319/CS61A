...
def interleaved_sum(n, odd_term, even_term):
    ...
    if n == 0:
        return 0
    return (odd_term if (n & 1) else even_term)(n) + interleaved_sum(n - 1, odd_term, even_term)
...
def mutate_reverse(link):
    ...
    def link2list(link):
        if link is Link.empty: return []
        return [link.first] + link2list(link.rest)
    def list2link(lst):
        if len(lst) == 0: return Link.empty
        return Link(lst[0], list2link(lst[1:]))
    result = list2link(list(reversed(link2list(link))))
    link.first, link.rest = result.first, result.rest
